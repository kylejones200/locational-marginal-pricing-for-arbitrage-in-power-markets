...
def load_and_prepare_data(year):
    ...
    nox = pd.to_numeric(
        df[[c for c in df.columns if "NOx" in c and "annual" in c and "tons" in c][0]],
        errors="coerce",
    )
    so2 = pd.to_numeric(
        df[[c for c in df.columns if "SO2" in c and "annual" in c and "tons" in c][0]],
        errors="coerce",
    )
...
def visualize_results(
    df, iso_pred, lof_pred, cov_pred, ensemble_mask, features, plot: bool = False
):
    ...
    if plot:
        ...
        def plot_scatter(ax, x_feat, y_feat, predictions, title):
            ...
            ax.scatter(
                df.loc[anomaly_mask, x_feat],
                df.loc[anomaly_mask, y_feat],
                c="#e74c3c",
                alpha=0.8,
                s=80,
                marker="X",
        ...
        ax7.hist(
            df.loc[~ensemble_mask, "carbon_intensity"],
            bins=50,
            alpha=0.7,
            color="#3498db",
            label="Normal",
        ...
        ax8.hist(
            df.loc[~ensemble_mask, "capacity_factor"],
            bins=50,
            alpha=0.7,
            color="#3498db",
            label="Normal",
